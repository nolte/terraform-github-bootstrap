...
- The GitHub App itself — registration and private-key generation — cannot be created by Terraform. Both are UI-only operations at `https://github.com/settings/apps/new` and the App's settings page. This repo only manages what happens **after** the App exists: per-repo Actions variable, secret, and (Phase 2) branch-protection bypass.
