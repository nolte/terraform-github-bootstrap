...
- Ruleset **bypass actors** are Terraform-owned and inherit from nothing: a bypass added through the GitHub UI reads as drift and the next apply deletes it. When a repo's release automation pushes the `chore(release): <tag>` commit with the portfolio App token, or cascades the tag into `master`/`main`, and its ruleset protects that branch (`require_pull_request = true` or required checks), declare `bypass_portfolio_app = true` on the ruleset and set `portfolio_app_id` (git-ignored tfvars, or `TF_VAR_portfolio_app_id` from gopass). Deleting that bypass breaks the release cascade. Repos using the `develop`-only, `require_pull_request = false` shape (`gh-plumbing`, `kamerplanter`) need no bypass. A `-target`ed apply is the safe move whenever an unrelated plan shows a `bypass_actors` removal.
