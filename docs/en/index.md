...
- **Docker Hub pull credential** — a read-only `DOCKERHUB_USERNAME` variable and `DOCKERHUB_TOKEN` secret on the repositories whose CI pulls images, so those pulls stop being rate-limited on a shared runner IP (see [Docker Hub Credential](dockerhub.md)).
...
terraform/
  ...
  dockerhub/            # Docker Hub read credential (variable + secret)
```
